 ...
 * (ie., no haga cosas como construir listas para evitar la manipulación de arreglos).
 */
public class SandboxArreglos
{
    ...
    public void eliminarEntero( int valor )
    {
    	...
    	for (int entero : arregloEnteros) {
    		if (entero != valor) {
    			nuevoArreglo[indice++] = entero;
    		}
    	}
    	int[] finalArreglo = new int[indice];
    	for (int i = 0; indice > i; i++) {
    		finalArreglo[i] = nuevoArreglo[i];
    	}
    	arregloEnteros = finalArreglo;
    }
    ...
    public void eliminarCadena( String cadena )
    {
    	...
    	for (String palabra : arregloCadenas) {
    		if (!palabra.equals(cadena)) {
    			nuevoArreglo[indice++] = palabra;
    		}
    	}
    	String[] finalArreglo = new String[indice];
    	for (int i = 0; indice > i; i++) {
    		finalArreglo[i] = nuevoArreglo[i];
    	}
    	arregloCadenas = finalArreglo;
    }
    	...
    	for (int i = 0; i < arregloEnteros.length - 1 ;i++) {
    		for (int j = 0 ; j < arregloEnteros.length - 1 - i; j++) {
    			 if (arregloEnteros[j] > arregloEnteros[j+1]) {
    				 int u = arregloEnteros[j];
    				 arregloEnteros[j] = arregloEnteros[j+1];
                     arregloEnteros[j+1] = u;
    			 }
    		}
    ...
    public int[] buscarEntero( int valor )
    {
    	...
    	int[] listaPosiciones = new int[indice];
    	for (int i = 0; i < indice; i++) {
    		listaPosiciones[i] = posiciones[i];
    	}
    ...
    public boolean mismosEnteros(int[] otroArreglo) {
        ...
        int[] copiaUno = new int[arregloEnteros.length];
        int[] copiaDos = new int[otroArreglo.length];
        for (int i = 0; i < arregloEnteros.length; i++) {
            copiaUno[i] = arregloEnteros[i];
            copiaDos[i] = otroArreglo[i];
        }
        for (int i = 0; i < copiaUno.length - 1; i++) {
            for (int j = i + 1; j < copiaUno.length; j++) {
                if (copiaUno[i] > copiaUno[j]) {
                    int temp = copiaUno[i];
                    copiaUno[i] = copiaUno[j];
                    copiaUno[j] = temp;
                }
            }
        }
        for (int i = 0; i < copiaDos.length - 1; i++) {
            for (int j = i + 1; j < copiaDos.length; j++) {
                if (copiaDos[i] > copiaDos[j]) {
                    int temp = copiaDos[i];
                    copiaDos[i] = copiaDos[j];
                    copiaDos[j] = temp;
                }
            }
        }
        for (int i = 0; i < copiaUno.length; i++) {
            if (copiaUno[i] != copiaDos[i]) {
                return false;
            }
        }
        return true;
